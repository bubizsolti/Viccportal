document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("morickaViccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Móricka viccek"); // Csak a Móricka viccek
                localStorage.setItem("moricka_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("morickaViccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("moricka_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Móricka viccek"); // Visszaállítás
