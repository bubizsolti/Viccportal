document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("cigany_viccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Cigány viccek"); // Csak a Cigány viccek
                localStorage.setItem("cigany_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("cigany_viccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("cigany_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Cigány viccek"); // Visszaállítás
