document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("morbidviccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Morbid viccek"); // Csak a Morbid viccek
                localStorage.setItem("morbid_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("morbidviccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("morbid_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Morbid viccek"); // Visszaállítás
