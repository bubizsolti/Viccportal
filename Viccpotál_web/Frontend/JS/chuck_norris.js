document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("chuckNorris_viccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Chuck Norris viccek"); // Csak a Chuck Norris viccek
                localStorage.setItem("chuckNorris_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("chuckNorris_viccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása az új viccekhez
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("chuckNorris_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Chuck Norris viccek"); // Visszaállítás
