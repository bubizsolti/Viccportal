...
async function fetchRandomJokes(page = 1) {
    try {
        // Lekérjük az "Állatos viccek" kategóriájú vicceket a `Viccportál` táblából
        const { data: jokesData, error } = await supabaseClient
            .from("Viccportál")
            .select("id, nev, vicc, ertekeles, ertekelesek_szama")
            .eq("kategoriak", "Állatos viccek") // Csak az "Állatos viccek" kategóriát kérjük
            .range((page - 1) * jokesPerPage, page * jokesPerPage - 1); // Lapozás

        if (error) {
            console.error("Hiba az 'Állatos viccek' lekérésekor:", error.message);
            return [];
        }

        return jokesData;
    } catch (error) {
        console.error("Hiba az 'Állatos viccek' lekérésekor:", error);
        return [];
    }
}
