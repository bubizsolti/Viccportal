document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("munkahely_viccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Munkahely viccek"); // Csak a Munkahely viccek
                localStorage.setItem("munkahely_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("munkahely_viccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása az új viccekhez
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("munkahely_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Munkahely viccek"); // Visszaállítás
