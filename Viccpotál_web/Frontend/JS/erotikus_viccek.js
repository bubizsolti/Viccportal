document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            const jokeHistory = []; // Vicc története

            // Ellenőrzés, hogy naponta frissítjük-e a vicceket
            if (localStorage.getItem("erotikus_viccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Erotikus viccek"); // Csak az Erotikus viccek
                localStorage.setItem("erotikus_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("erotikus_viccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("erotikus_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Erotikus viccek"); // Visszaállítás
