document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("szo_viccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Szóviccek"); // Csak a Szóviccek
                localStorage.setItem("szo_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("szo_viccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása az új viccekhez
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("szo_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Szóviccek"); // Visszaállítás
