...
async function fetchRandomJokes(page = 1) {
    try {
        // Lekérjük az "Morbid viccek" kategóriájú vicceket a `Viccportál` táblából
        const { data: jokesData, error } = await supabaseClient
            .from("Viccportál")
            .select("id, nev, vicc, ertekeles, ertekelesek_szama")
            .eq("kategoriak", "Morbid viccek") // Csak az "Morbid viccek" kategóriát kérjük
            .range((page - 1) * jokesPerPage, page * jokesPerPage - 1); // Lapozás

        if (error) {
            console.error("Hiba az 'Morbid viccek' lekérésekor:", error.message);
            return [];
        }

        return jokesData;
    } catch (error) {
        console.error("Hiba az 'Morbid viccek' lekérésekor:", error);
        return [];
    }
}
