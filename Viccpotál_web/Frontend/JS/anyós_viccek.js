document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("anyos_viccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Anyós viccek"); // Csak az Anyós viccek
                localStorage.setItem("anyos_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("anyos_viccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása az új viccekhez
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("anyos_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Anyós viccek"); // Visszaállítás
