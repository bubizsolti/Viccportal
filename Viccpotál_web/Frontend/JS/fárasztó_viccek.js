document.addEventListener("DOMContentLoaded", function() {
    fetch('../JSON/jokes.json')
        ...
        .then(jokes => {
            ...
            if (localStorage.getItem("farasztoviccekDisplayDate") !== today) {
                currentJokesStack = jokes.filter(joke => joke.category === "Fárasztó viccek"); // Csak a Fárasztó viccek
                localStorage.setItem("farasztó_displayedJokes", JSON.stringify(currentJokesStack));
                localStorage.setItem("farasztoviccekDisplayDate", today);
                currentIndex = 0; // Index visszaállítása
            } else {
                currentJokesStack = JSON.parse(localStorage.getItem("farasztó_displayedJokes")); // Meglévő viccek betöltése
            }
            ...
            document.getElementById("next-jokes-button").addEventListener("click", function(event) {
                ...
                if (currentIndex >= currentJokesStack.length) {
                    ...
                    currentJokesStack = jokes.filter(joke => joke.category === "Fárasztó viccek"); // Visszaállítás
