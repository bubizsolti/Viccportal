...
async function fetchRandomJokes(page = 1) {
    try {
        // Lekérjük az "Fárasztó viccek" kategóriájú vicceket a `Viccportál` táblából
        const { data: jokesData, error } = await supabaseClient
            .from("Viccportál")
            .select("id, nev, vicc, ertekeles, ertekelesek_szama")
            .eq("kategoriak", "Fárasztó viccek") // Csak az "Fárasztó viccek" kategóriát kérjük
            .range((page - 1) * jokesPerPage, page * jokesPerPage - 1); // Lapozás

        if (error) {
            console.error("Hiba az 'Fárasztó viccek' lekérésekor:", error.message);
            return [];
        }

        return jokesData;
    } catch (error) {
        console.error("Hiba az 'Fárasztó viccek' lekérésekor:", error);
        return [];
    }
}
