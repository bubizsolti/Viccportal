...
async function fetchRandomJokes() {
    try {
        // Lekérjük a vicceket a `Viccportál` táblából, ABC sorrendben
        const { data: jokesData, error } = await supabaseClient
            .from("Viccportál")
            .select("id, nev, vicc, ertekeles, ertekelesek_szama")
            .order("nev", { ascending: true }) // Növekvő sorrendben rendezzük a "nev" (vicc neve) mező alapján
            .limit(10); // Csak az első 10 viccet kérjük le

        if (error) {
            console.error("Hiba a viccek lekérésekor:", error.message);
            return [];
        }

        return jokesData;
    } catch (error) {
        console.error("Hiba a viccek lekérésekor:", error);
        return [];
    }
}
